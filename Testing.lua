...
local Color3_fromRGB, Instance_new, loadstring, UDim2_fromOffset, UDim2_new, Vector2_new, isfolder, makefolder, isfile, pcall, table_concat, table_insert, writefile, readfile, string_gmatch, delfile, ipairs, print, pairs, table_remove, task_spawn, task_wait, warn = Color3.fromRGB, Instance.new, loadstring, UDim2.fromOffset, UDim2.new, Vector2.new, isfolder, makefolder, isfile, pcall, table.concat, table.insert, writefile, readfile, string.gmatch, delfile, ipairs, print, pairs, table.remove, task.spawn, task.wait, warn
...
local Window = Library:CreateWindow{
	...
	TabWidth = 160,
	Size = UDim2_fromOffset(830, 525),
	Resize = true,
	MinSize = Vector2_new(470, 380),
	Acrylic = true,
	Theme = "Dark",
}
...
local externalButton = Instance_new("TextButton")
externalButton.Parent = game.Players.LocalPlayer.PlayerGui
externalButton.Size = UDim2_new(0, 150, 0, 50) -- Adjust size as needed
externalButton.Position = UDim2_new(0.5, -75, 0, 100) -- Position it somewhere visible on the screen
externalButton.Text = "Toggle Interface"
externalButton.TextColor3 = Color3_fromRGB(255, 255, 255)
externalButton.BackgroundColor3 = Color3_fromRGB(0, 122, 255)
...
local function loadAuraListFromFile()
	...
	if isfile(filePath) then
		...
		for line in string_gmatch(fileContents, "[^\r\n]+") do
			table_insert(auraList, line)
		end
	end
...
local function saveAuraListToFile()
	...
	writefile(filePath, table_concat(aurasToDelete, "\n"))
...
local function addOrRemoveAura()
	...
	if auraName and auraName ~= "" then
		...
		if not found then
			...
			table_insert(aurasToDelete, auraName)
			Library:Notify{
				Title = "Aura Added",
